public class Prisoner extends Thread {
    ...
    public Prisoner(int id, Room room, Warden warden,Cell mycell){
        this.id = id;
        this.room = room;
        this.warden = warden;
        this.mycell = mycell;
    ...
    synchronized void live() throws InterruptedException {
        while (true) {
            ...
            while (this.mycell.isTheDoorClosed())
                this.waitForCellDoorToOpen(); // waitUntilInCell() will proceed on this line of code
    ...
    public synchronized void waitForCellDoorToOpen(){
      try{
        while(this.mycell.isTheDoorClosed()){
          this.wait();
        }
